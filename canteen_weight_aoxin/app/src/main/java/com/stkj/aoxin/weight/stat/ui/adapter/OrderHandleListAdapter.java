...
public class OrderHandleListAdapter extends BaseQuickAdapter<OrderItemBean.RecordsBean, BaseViewHolder> {

    private static final String TAG = "OrderHandleListAdapter";
    private Context context;

    public OrderHandleListAdapter(Context context)
    {
        super(R.layout.item_order_handle_list_goods);
    ...
    protected void convert(BaseViewHolder holder, OrderItemBean.RecordsBean item) {
        ...
        holder.setText(R.id.tv_order_username, item.getCreateUserName());
        holder.setText(R.id.tv_order_create_time, item.getAcceptOrdersTime());
        holder.setText(R.id.tv_commany, item.getSupplierName());
